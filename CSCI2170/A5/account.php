...
$pwRule="Password need to be at least 7 characters long with at least one number, one capital letter, and uses at least a _ or $ character.";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $status==1){//if it's sign up page
    ...
    if (empty($_POST["password"]))
    {
        ...
        if (strlen($password)<7)//password less than 7 character
        {
            $pwError = "<strong>Sorry, the password is too short.</strong><br>".$pwRule;
        }
        //pattern: at least 7 characters that has at least one number, one capital letter, and uses at least one of the following two special characters (  _  $ )
        else if (!preg_match('/^(?=.*[A-Z])(?=.*\d)(?=.*[_\$])[A-Za-z\d\$_]{7,}/',$password,$match) ) {
            if (preg_match('/^(?=.*[A-Z])(?=.*[_\$])[A-Za-z\$_]{7,}/',$password,$match))//has no number
            {
                $pwError = "<strong>Sorry, the password needs at least one number.</strong><br>".$pwRule;
            }
            elseif (preg_match('/^(?=.*\d)(?=.*[_\$])[a-z\d\$_]{7,}/',$password,$match))//no capital letter
            {
                $pwError ="<strong>Sorry, the password needs at least one capital letter.</strong><br>".$pwRule;
            }
            else if (preg_match('/^(?=.*\d)(?=.*[A-Z])(?=.*\d)[A-Za-z\d]{7,}/',$password,$match))//no symbol
            {
                $pwError ="<strong>Sorry, the password needs at least a _ or $ character.</strong><br>".$pwRule;
            }
            else if (preg_match('/^[A-Z]{7,}/',$password,$match))//has only capital characters
            {
                $pwError ="<strong>Sorry, the password needs at least one number and at least a _ or $ character.</strong><br>".$pwRule;
            }
            else if (preg_match('/\d{7,}/',$password,$match))//only has numbers
            {
                $pwError ="<strong>Sorry, the password needs at least one capital letter and at least a _ or $ character.</strong><br>".$pwRule;
            }
            else
            {
                $pwError ="<strong>Sorry, $pwRule</strong>";//has only small characters or other circumstances
            }
            $password="";
            $pwCheck=false;
        }
    ...
    if ($usernameCheck==true && $pwCheck==true)
    {
        ...
        $hashedPW=password_hash($password,PASSWORD_DEFAULT);//hashed the password
        addLogin($id,$name,$hashedPW);//add in log in table
    ...
    else
    {
        ...
        $try="<br><strong>Please try again</strong>";//error message
        $status=1;
    }
}

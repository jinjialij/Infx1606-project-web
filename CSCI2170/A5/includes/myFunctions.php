...
function search()
{
    ...
    //$myquery="select Name, Date,Post from chat.posts join chat.user using (UserID) where UserID=".$id." order by date desc,Name asc";

    $row=0;
    $col=0;
    for ($i=0;$i<count($id);$i++)
    {
        $myquery="select Name, Date, Post from chat.posts join chat.user using (UserID) where UserID=".$id[$i]." order by date desc,Name asc";
        $result1 = mysqli_query ($conn,$myquery);
        if ($result1 = $conn->query($myquery)) {
            while($row=$result1->fetch_assoc())
            {
                $searchPost[$row][$col]=$row['Name'];
                $searchPost[$row][$col+1]=$row['Date'];
                $searchPost[$row][$col+2]=$row['Post'];
                $row++;
            }
        }
        else {
...
{
    $searchUserIdList=array();
    $searchUserIdList[0]=1;//add my id in the id list
    ...
    //queries that gets IDs of followings and followers
    $query1="select FollowingUser from chat.following where User=1";
    $query2="select User from chat.following where FollowingUser=1";
    ...
    if ($result1 = $conn->query($query1)) {
        while($row=$result1->fetch_assoc())
        {
            $searchUserIdList[$index++]=$row["FollowingUser"];
        }
    }
    else {
        echo "Nothing here to display! Sorry!";
    }
    $index=count($searchUserIdList);
    if ($result2 = $conn->query($query2)) {
        while($row=$result2->fetch_assoc())
        {
            $searchUserIdList[$index++]=$row["User"];
        }
    }
    else {
        echo "Nothing here to display! Sorry!";

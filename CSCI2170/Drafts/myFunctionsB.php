 ...
 * Time: 10:58
 */
//user 1 is Marty, user2 is Ty Lyse, user3 is Luke
//load all my chats
function loadMyChats()
{
    ...
    if($mCLength!=0)
    {
        ...
        {
            for ($row=0;$row<(($mCLength-1)/4);$row++)
            {
...
function loadFollowingChats()
{
    ...
    $chats = array(
        array($user1Lines[0], $user1Lines[1], $user1Lines[4], $user1Lines[5]),//user1 chat1 array name time timestamp text
        array($user1Lines[0], $user1Lines[7], $user1Lines[8], $user1Lines[9]),
        array($user1Lines[0], $user1Lines[11], $user1Lines[12], $user1Lines[13]),

        //user2 chats arrays
        array($user2Lines[0], $user2Lines[1], $user2Lines[4], $user2Lines[5]),//user2 chat1 array(name time timestamp text)
        array($user2Lines[0], $user2Lines[7], $user2Lines[8], $user2Lines[9]),
        array($user2Lines[0], $user2Lines[11], $user2Lines[12], $user2Lines[13]),

        //user3 chats arrays
        array($user3Lines[0], $user3Lines[1], $user3Lines[2], $user3Lines[3]),//user3 chat1 array
...
//load chats determined by the value the $_GET get
...
function loadDesc($getName)
{
    ...
    else if ($getName=="myChat")
    {
        ...
        $myDesc = fread($fh,filesize("./my_profile.txt"));
        fclose($fh);
        return $myDesc;
    }
    else if ($getName=="Ty Lysa")

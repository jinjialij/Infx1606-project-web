...
function myNewChats($newChatText,$userID)
{
    ...
    if($newChatText!=null)//write file only if it is not null
    {
        ...
        $date = date_create();
        $dateTime = date("Y-m-d H:i:s", time());//changed because of the feedback of assignment 4: fixed insert datetime 
        //connect database
        $login=login();
        //connect database
        $conn=new mysqli($login[0],$login[1],$login[2],$login[3]);
...
function addLogin($userID,$name,$hpassword)
{
    //connect database
    $login=login();
    //connect database
    ...
    $stmt->bind_param("iss", $userID,$name,$hpassword);
    if ($stmt->execute()) {
        // it worked
    } else {// it didn't
        echo "Error";
...
//check if inputted name and password(hashed) match and return results:userid,name,error info
function loginCheck($name,$password){
    ...
    //sql query that return results according to user name
    $stmt = $conn->prepare("select userID,username,password from login where username= ? ");
    $stmt->bind_param("s", $name);
    $stmt->execute();
    /* Store the result (to get properties) */
    $stmt->store_result();
    ...
    {
        $check[2]="The username is incorrect";
    }
